...
def main_from_folder(args, ):
    ...
    ds_path = '/dataset/images_thermal_val/data'
    
    jpg_files = sorted(glob.glob(os.path.join(ds_path, '**', '*.jpg'), recursive=True))
    for jpg_path in jpg_files:
        im_pil = Image.open(jpg_path).convert('RGB')
        w, h = im_pil.size
        orig_size = torch.tensor([w, h])[None].to(args.device)
        ...
        out_path = os.path.join(output_dir, os.path.splitext(base_name)[0] + '_result.jpg')
        draw([im_pil], labels, boxes, scores, 0.6, out_path)
...
if __name__ == '__main__':
    ...
    # it allows RT-DETR’s scripts to load and continue from a previously saved model checkpoint. Here's how it works:
